...
qint64 SerialIf_Write(const QByteArray& data) {
	qDebug().nospace() << __FILE__ << "(" << __LINE__ << ") SerialIf_Write(" << data << ")";
	if (m_Interface==IF_SERIAL) {
...
QByteArray SerialIf_ReadLine() {
	if (m_Interface==IF_SERIAL) {
		QByteArray qByteArray = m_serialPort.readLine();
		qDebug().nospace() << __FILE__ << "(" << __LINE__ << ") SerialIf_ReadLine() -> " << qByteArray;
		return qByteArray;
	} else if (m_Interface==IF_ETHERNET) {

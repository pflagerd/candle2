...
public:
	...
	static const int waitForUnlockReminderNonqueriedFeedbackMessage = 3;	// b"[MSG:'$H'|'$X' to unlock]\r\n" // This is called a Non-queried feedback message <a href="https://github.com/pflagerd/grbl/blob/e5e10b5b0d1d571da7ff85b4265b4773736aef01/doc/markdown/interface.md?plain=1#L105">defined here.</a> <a href="https://github.com/pflagerd/grbl/blob/e5e10b5b0d1d571da7ff85b4265b4773736aef01/doc/markdown/interface.md?plain=1#L339">See Here for specifics on the Unlock reminder</a>
};
...
void frmMain::ProcessGRBL1_1() {
    while (SerialIf_CanReadLine()) {
        ...
        if (m_isAwaitingGrblVersionString) { // DPP: is the system in a state where it's waiting for the Grbl Version String which is
            ...
            if (!wasGrblVersionStringReceived(data)) // does <code>data</code> contain a Grbl Version String which looks something like "Grbl 1.1h ['$' for help]"
                continue;
            else {
        ...
        if (data[0] == '<') {
            ...
            if (stx.indexIn(data) != -1) {
                ...
                if (m_aborting) {
                    switch (status) {
                        ...
                        case HOLD1:
                        case QUEUE:
                            if (!m_isAwaitingGrblVersionString && compareCoordinates(x, y, z)) {
                                x = sNan;
                                y = sNan;
                                z = sNan;
                                a = sNan;
                            ...
                            break;
						default:
							qDebug().nospace() << __FILE__ << "(" << __LINE__ << ") Unexpected status " << status;
							break;
                    }
                }
            }
            ...
            // Processed commands
            //if(m_CommandAttributesList.length() > 0 && !DataIsFloating(data) && !(m_CommandAttributesList[0].command != "[CTRL+X]" && wasGrblVersionStringReceived(data)))
            auto &val = mCommandsSent.front();

            if (mCommandsSent.size() > 0 && !DataIsFloating(data) &&
                !(val.command != "[CTRL+X]" && wasGrblVersionStringReceived(data))) {
                static QString response; // Full response string

                if ((mCommandsSent.front().command != "[CTRL+X]" && DataIsEnd(data)) ||
                    (mCommandsSent.front().command == "[CTRL+X]" && wasGrblVersionStringReceived(data))) {
...
void frmMain::ProcessGRBL_ETH(const QString& data_) {
	...
	if (m_isAwaitingGrblVersionString) {
